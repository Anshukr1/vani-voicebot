...
const App: React.FC = () => {
      ...
      <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center justify-center p-4 font-sans">
        <div className="max-w-md w-full bg-slate-800 p-8 rounded-3xl shadow-2xl border border-slate-700">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">Top Interests (comma separated)</label>
              <input 
                type="text" 
  ...
  if (!selectedTopic) {
    // Extract topics from profession, investments, and interests
    const rawString = `${userProfile.profession}, ${userProfile.investments}, ${userProfile.interests}`;
    const parts = rawString.split(/[,;\n]|\b and \b|-/i);
    const topics = parts
      .map(p => p.trim())
      .filter(p => p.length > 2)
      .filter(p => p.toLowerCase() !== 'none' && p.toLowerCase() !== 'nothing' && p.toLowerCase() !== 'na');
    
    // Remove duplicates (case-insensitive)
    const interestsList = Array.from(new Set(topics.map(t => t.toLowerCase()))).map(
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden">
        ...
        <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
          <div className="absolute -top-[20%] -right-[20%] w-[600px] h-[600px] bg-indigo-900/20 rounded-full blur-[100px] opacity-50" />
          <div className="absolute top-[40%] -left-[20%] w-[500px] h-[500px] bg-rose-900/20 rounded-full blur-[100px] opacity-50" />
        </div>

        <div className="relative z-10 max-w-2xl w-full bg-slate-800/80 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-slate-700">
          <div className="flex items-center space-x-3 mb-8 justify-center">
            <div className="w-12 h-12 bg-gradient-to-br from-indigo-500 to-rose-500 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-500/20">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" />
          ...
          <h2 className="text-xl font-medium mb-8 text-center text-slate-300">What would you like to hear about today?</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <button
              onClick={() => setSelectedTopic('Combined')}
              className="bg-gradient-to-r from-indigo-600 to-rose-600 hover:from-indigo-500 hover:to-rose-500 text-white font-bold py-6 px-6 rounded-xl shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98] flex flex-col items-center justify-center text-center group"
            >
              ...
              <span className="text-sm font-normal opacity-80 group-hover:opacity-100 transition-opacity">General briefing across all topics</span>
            </button>
            ...
            {interestsList.map((interest, idx) => (
              <button
                ...
                className="bg-slate-700/50 hover:bg-slate-700 text-white font-bold py-6 px-6 rounded-xl shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98] border border-slate-600 flex flex-col items-center justify-center text-center group"
              >
                <span className="text-xl mb-2 flex items-center gap-2 text-indigo-300 group-hover:text-indigo-200 transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" />
                  </svg>
                  {interest}
                </span>
                <span className="text-sm font-normal text-slate-400 group-hover:text-slate-300 transition-colors">Deep dive into {interest}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <header className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
             <button 
               ...
               className="w-10 h-10 bg-slate-800 hover:bg-slate-700 rounded-xl flex items-center justify-center shadow-lg border border-slate-700 transition-colors"
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{selectedTopic === 'Combined' ? 'General News' : selectedTopic}</p>
            </div>
          </div>
